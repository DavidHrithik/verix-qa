...
export const seedTestCases: TestCase[] = [
  ...
  },
  {
    ...
    storyId: 'story-auth-101',
    key: 'TC-103',
    title: 'Verify password field enforces 8-character minimum boundary threshold',
    type: 'Automated',
    priority: 'High',
    status: 'Approved',
    isAiGenerated: true,
    aiConfidence: 94,
    tags: ['Auth', 'Boundary', 'Threshold'],
    lastExecutionStatus: 'Passed',
    steps: [
      { stepNumber: 1, action: 'Enter 7-character password "Pass12!" (1 below minimum)', expectedResult: 'Boundary threshold evaluated in real-time' },
      { stepNumber: 2, action: 'Focus shifts from password input field', expectedResult: 'Validation rule triggers boundary helper tooltip' },
      { stepNumber: 3, action: 'Assert helper text enforces "Password must be at least 8 characters"', expectedResult: '8-character threshold strictly enforced' },
    ],
  ...
  },
  {
    ...
    storyId: 'story-auth-101',
    key: 'TC-104',
    title: 'Verify duplicate email submission returns HTTP 409 Conflict with login redirect',
    type: 'Automated',
    priority: 'High',
    status: 'Approved',
    isAiGenerated: true,
    aiConfidence: 96,
    tags: ['Auth', 'Edge-Case', 'Conflict'],
    lastExecutionStatus: 'Passed',
    steps: [
      { stepNumber: 1, action: 'Submit registration with pre-existing email "alex.morgan@company.com"', expectedResult: 'Backend returns HTTP 409 Conflict status' },
      { stepNumber: 2, action: 'Assert warning banner displays 409 Conflict with login link', expectedResult: 'Conflict banner rendered with login redirect link' },
    ],
  ...
  },
  {
    ...
    storyId: 'story-auth-101',
    key: 'TC-105',
    title: 'Verify input sanitization strips malicious HTML/script tags in name field (OWASP XSS)',
    type: 'Automated',
    priority: 'Critical',
    status: 'Approved',
    isAiGenerated: true,
    aiConfidence: 99,
    tags: ['Auth', 'Security', 'OWASP', 'XSS'],
    lastExecutionStatus: 'Passed',
    steps: [
      { stepNumber: 1, action: 'Enter malicious XSS payload in Full Name field', expectedResult: 'Raw payload entered in DOM input' },
      { stepNumber: 2, action: 'Submit form through OWASP security filter', expectedResult: 'Filter strips script tags and escapes HTML entities' },
      { stepNumber: 3, action: 'Assert name is sanitized to "Alex Morgan" with zero script execution', expectedResult: 'XSS protection verified with zero vulnerability' },
    ],
