...
<head>
  <meta charset="UTF-8">
  <title>ExtentReports — ${escapeHtml(storyKey)} Autonomous BDD Suite</title>
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media print {
      ...
      .extent-container { box-shadow: none !important; border: 1px solid #cbd5e1 !important; background: #ffffff !important; }
...
<body>
  <div class="top-toolbar">
    <div style="font-size: 13px; color: #94a3b8;">
      📄 <strong>ExtentReports 5.1 Document View</strong> (${escapeHtml(storyKey)})
    </div>
  ...
  <div class="extent-container">
    ...
    <div class="content-body">
      <div class="feature-card">
        <div class="feature-header">
          <div>
            <h2 class="feature-title">Feature: ${escapeHtml(title)}</h2>
            <div class="tag-cloud">
              ${(script.featureTags || ['@BDD', '@Regression', `@${storyKey.replace('-', '')}`, '@AutonomousQA']).map(t => `<span class="tag-pill">${escapeHtml(t)}</span>`).join('')}
            </div>
          </div>
        ...
        <div class="scenario-container">
          ${subScenarios ? subScenarios.map((scen, scenIdx) => `
            <div style="margin-bottom: 24px; padding-bottom: 20px; border-bottom: ${scenIdx < subScenarios.length - 1 ? '1px solid #334155' : 'none'};">
              <div class="scenario-title">
                <span style="color: #38bdf8; font-family: monospace;">[${escapeHtml(scen.testCaseKey)}]</span>
                <span>${escapeHtml(scen.title)}</span>
                ...
                ${scen.steps.map(st => `
                  <div class="step-item">
                    <div class="step-header">
                      <div>
                        <span class="step-keyword">${escapeHtml(st.keyword || 'Given')}</span>
                    ...
                    <div class="step-action-meta">Expected: ${escapeHtml(st.expectedResult)}</div>
                    ${st.locator ? `<div class="step-action-meta" style="color: #38bdf8;">Locator: ${escapeHtml(st.locator)}</div>` : ''}
                  </div>
                `).join('')}
              </div>
            </div>
          `).join('') : `
            <div class="scenario-title">
              <span style="color: #38bdf8; font-family: monospace;">[${escapeHtml(testCaseKey)}]</span>
              <span>Scenario: ${escapeHtml(script.testCaseTitle || 'Primary Scenario Verification')}</span>
              <span class="step-badge-pass">PASS</span>
            </div>
            <div>
              ${script.steps.map(st => `
    ...
    // Create safe Blob URL and open directly in a new tab
...
<head>
  <meta charset="UTF-8">
  <title>Verix QA — ${escapeHtml(storyKey)} Executive Audit Report</title>
  <style>
  ...
  <div class="container">
    ...
    <div class="meta-grid">
      <div class="meta-card">
        ...
        <div class="meta-value">${escapeHtml(storyKey)} — ${escapeHtml(title)}</div>
      </div>
      <div class="meta-card">
    ...
    ${script.steps.map(st => `
      <div style="background:#0F172A; border:1px solid #334155; padding:12px; border-radius:6px; margin-bottom:8px;">
        <div style="display:flex; justify-content:space-between; font-weight:600; font-size:13px;">
          <span>${st.stepNumber}. <strong style="color:#38BDF8;">${escapeHtml(st.keyword || 'Given')}</strong> ${escapeHtml(st.title)}</span>
          <span style="color:#34D399; font-size:11px;">PASSED ✓</span>
        </div>
        <div style="font-size:12px; color:#94A3B8; margin-top:4px;">Expected: ${escapeHtml(st.expectedResult)}</div>
      </div>
    `).join('')}
    <div class="footer">
